...
class SEGAN(Model):
    ...
    def build_model_single_gpu(self, gpu_idx):
        if gpu_idx == 0:
            ...
            self.get_wav, self.get_noisy = read_and_decode(filename_queue, self.canvas_size, self.preemph)
        # load the data to input pipeline
        wavbatch, noisybatch = tf.train.shuffle_batch([self.get_wav, self.get_noisy], batch_size=self.batch_size,
        ...
        D_fk_joint = tf.concat(3, [G, noisybatch])
        ...
        self.rl_audio_summ = tensor_summary('real_audio', wavbatch)
        self.real_w_summ = tensor_summary('real_wav', wavbatch)
        self.noisy_audio_summ = tensor_summary('noisy_audio', noisybatch)
        self.noisy_w_summ = tensor_summary('noisy_wav', noisybatch)
        self.gen_audio_summ = tensor_summary('G_audio', G)
        self.gen_summ = tensor_summary('G_wav', G)
    ...
    def train(self, config, devices):
        ...
        self.sess.run(init)
        g_summs = [self.d_fk_sum,  # self.d_nfk_sum,
                   self.d_fk_loss_sum,  # self.d_nfk_loss_sum,
                   self.g_loss_sum, self.g_loss_l1_sum, self.g_loss_adv_sum, self.gen_summ, self.gen_audio_summ]
        ...
        self.d_sum = tf.summary.merge(
            [self.d_loss_sum, self.d_rl_sum, self.d_rl_loss_sum, self.rl_audio_summ, self.real_w_summ,
        ...
        try:
            while not coord.should_stop():
                ...
                print('{}/{} (epoch {}), d_rl_loss = {:.5f},d_fk_loss = {:.5f},'
                      ...
                      ' time/batch = {:.5f}, '
                      'mtime/batch = {:.5f}'.format(counter, config.epoch * num_batches, curr_epoch, d_rl_loss,
                                                    d_fk_loss,  # d_nfk_loss,
                                                    g_adv_loss, g_l1_loss, end - start, np.mean(batch_timings)))
...
class SEAE(Model):
    ...
    def build_model_single_gpu(self, gpu_idx):
        if gpu_idx == 0:
            # create the nodes to load for input pipeline
            filename_queue = tf.train.string_input_producer([self.e2e_dataset])
            self.get_wav, self.get_noisy = read_and_decode(filename_queue, 128)
        # load the data to input pipeline
        wavbatch, noisybatch = tf.train.shuffle_batch([self.get_wav, self.get_noisy], batch_size=self.batch_size,
                                                      num_threads=2, capacity=1000 + 3 * self.batch_size,
                                                      min_after_dequeue=1000, name='wav_and_noisy')
    ...
    def train(self, config, devices):
        ...
        self.g_sum = tf.summary.merge(
            [self.g_loss_sum, self.gen_summ, self.rl_audio_summ, self.real_w_summ, self.gen_audio_summ])
        ...
        sample_noisy = self.gtruth_noisy[0]
        sample_wav = self.gtruth_wavs[0]
        print('sample noisy shape: ', sample_noisy.shape)
        print('sample wav shape: ', sample_wav.shape)
        save_path = config.save_path
        counter = 0
        ...
        try:
            while not coord.should_stop():
                ...
                print('{}/{} (epoch {}), g_loss = {:.5f},'
                      ' time/batch = {:.5f}, '
                      'mtime/batch = {:.5f}'.format(counter, config.epoch * num_batches, curr_epoch, g_loss,
                                                    end - start, np.mean(batch_timings)))
                batch_idx += num_devices
                counter += num_devices
                if (counter / num_devices) % config.save_freq == 0:
                    self.save(config.save_path, counter)
