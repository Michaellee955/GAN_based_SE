...
def downconv(x, output_dim, kwidth=4, kheight=4, stride=2, init=None, uniform=False, bias_init=None, name='downconv'):
    ...
    with tf.variable_scope(name):
        ...
        #conv = tf.reshape(conv, conv.get_shape().as_list()[:3] + [conv.get_shape().as_list()[-1]])
        return conv
...
def highway(input_, size, layer_size=1, bias=-2, f=tf.nn.relu, name='hw'):
    ...
    for idx in range(layer_size):
        ...
        transform_gate = tf.sigmoid(tf.nn.rnn_cell._linear(input_, size, 0, scope=transform_scope) + bias)
...
def prelu(x, name='prelu', ref=False):
    ...
    with tf.variable_scope(name):
        ...
        alpha = tf.get_variable('alpha', in_shape[-1], initializer=tf.constant_initializer(0.), dtype=tf.float32)
        pos = tf.nn.relu(x)
...
def atrous_conv1d(value, dilation, kwidth=3, num_kernels=1, name='atrous_conv1d', bias_init=None, stddev=0.02):
    ...
    with tf.variable_scope(name):
        ...
        filter_ = tf.get_variable('w', [kwidth, in_channels, num_kernels], initializer=weights_init, )
...
def residual_block(input_, dilation, kwidth, num_kernels=1, bias_init=None, stddev=0.02, do_skip=True,
                   name='residual_block'):
    print('input shape to residual block: ', input_.get_shape())
    with tf.variable_scope(name):
        h_a = atrous_conv1d(input_, dilation, kwidth, num_kernels, bias_init=bias_init, stddev=stddev)
        h = tf.tanh(h_a)
        # apply gated activation
        z_a = atrous_conv1d(input_, dilation, kwidth, num_kernels, name='conv_gate', bias_init=bias_init, stddev=stddev)
        z = tf.nn.sigmoid(z_a)
        ...
        # make res connection
        h_ = conv1d(gated_h, kwidth=1, num_kernels=1, init=tf.truncated_normal_initializer(stddev=stddev),
                    name='residual_conv1')
        res = h_ + input_
        print('residual result: ', res.get_shape())
        if do_skip:
            # make skip connection
            skip = conv1d(gated_h, kwidth=1, num_kernels=1, init=tf.truncated_normal_initializer(stddev=stddev),
                          name='skip_conv1')
            return res, skip
        else:
            return res
...
# TODO deconv checkboaerd problem to be solved
def nn_deconv(x, kwidth=5, dilation=2, init=None, uniform=False, bias_init=None, name='nn_deconv1d'):
    ...
    dec = conv1d(interp_x, kwidth=5, num_kernels=1, init=init, uniform=uniform, bias_init=bias_init, name=name,
                 padding='SAME')
    return dec


def deconv(x, output_shape, kwidth=4, kheight=4, dilation=2, init=None, uniform=False, bias_init=None, name='deconv1d'):
    input_shape = x.get_shape()
    in_channels = input_shape[-1]
