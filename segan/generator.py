    ...
    def __call__(self, noisy_w, is_ref, spk=None):
        ...
        for block_idx, dilation in enumerate(segan.g_dilated_blocks):
            ...
            if block_idx >= len(segan.g_dilated_blocks) - 1:
                skip_out = False
            if skip_out:
                res_i, skip_i = residual_block(h_i, dilation, kwidth, num_kernels=32, bias_init=None, stddev=0.02,
                                               do_skip=True, name=name)
            else:
                res_i = residual_block(h_i, dilation, kwidth, num_kernels=32, bias_init=None, stddev=0.02,
                                       do_skip=False, name=name)
        ...
        with tf.variable_scope('g_wave_pooling'):
            ...
            wave_a = conv1d(skips_sum, kwidth=1, num_kernels=1, init=tf.truncated_normal_initializer(stddev=0.02))
            wave = tf.tanh(wave_a)
...
class AEGenerator(object):
    ...
    def __call__(self, noisy_w, is_ref, spk=None, z_on=False, do_prelu=False):
        ...
        with tf.variable_scope('g_ae'):
            ...
            # enc ~ [128x128x1, 64x64x16, 32x32x32, 16x16x32, 8x8x64, 4x4x64, 2x2x128, 1x1x256]
            # dec ~ [1x1x256,2x2x128,4x4x64,8x8x64,16x16x32,32x32x32,64x64x16,128x128x1]
            # FIRST ENCODER
            for layer_idx, layer_depth in enumerate(segan.g_enc_depths):
                ...
                h_i_dwn = downconv(h_i, layer_depth, kwidth=kwidth, kheight=kheight,
                                   init=tf.truncated_normal_initializer(stddev=0.02),
                                   bias_init=bias_init, name='enc_{}'.format(layer_idx))
                if is_ref:
            ...
            for layer_idx, layer_depth in enumerate(g_dec_depths):
                ...
                out_shape = [h_i_dim[0], h_i_dim[1] * 2, h_i_dim[2] * 2, layer_depth]
                bias_init = None
                # deconv
                if segan.deconv_type == 'deconv':
                    ...
                    if segan.bias_deconv:
                        bias_init = tf.constant_initializer(0.)
                    h_i_dcv = deconv(h_i, out_shape, kwidth=kwidth, kheight=kheight, dilation=2,
