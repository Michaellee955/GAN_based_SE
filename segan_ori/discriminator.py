...
def discriminator(self, wave_in, reuse=False):
    ...
    with tf.variable_scope('d_model') as scope:
        ...
        def disc_block(block_idx, input_, kwidth, kheight, nfmaps, bnorm, activation, pooling=2):
            with tf.variable_scope('d_block_{}'.format(block_idx)):
                ...
                hi_a = downconv(input_, nfmaps, kwidth=kwidth, kheight=kheight,stride=pooling, init=downconv_init, bias_init=bias_init)
                if not reuse:
        ...
        if self.bias_D_conv:
            if not reuse:
                print('biasing D conv', end=' *** ')
            bias_init = tf.constant_initializer(0.25)
        downconv_init = tf.truncated_normal_initializer(stddev=0.02)
