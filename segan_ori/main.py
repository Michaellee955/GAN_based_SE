...
flags.DEFINE_integer("l1_remove_epoch", 150, "Epoch where L1 in G is "
                                             "removed (Def: 150).")
flags.DEFINE_boolean("bias_deconv", False, "Flag to specify if we bias deconvs (Def: False)")
flags.DEFINE_boolean("bias_downconv", False, "flag to specify if we bias downconvs (def: false)")
flags.DEFINE_boolean("bias_D_conv", False, "flag to specify if we bias D_convs (def: false)")
...
flags.DEFINE_float("init_l1_weight", 3., "Init L1 lambda (Def: 3)")
...
flags.DEFINE_string("models", "gan", "Type of models to train: gan or ae. (Def: gan).")
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        if FLAGS.models == 'gan':
            ...
            raise ValueError('{} models type not understood!'.format(FLAGS.model))
